...
module.exports = {
  extra_food_per_day: ({spaceId, daysAgo}, args, context) => {
    return knex
      .select('*')
      .from(function() {
  ...
  feeding_count_per_day: ({spaceId, daysAgo}, args, context) => {
    return knex
      .select('*')
      .from(function() {
  ...
  night_breaks: ({spaceId, daysAgo}, args, context) => {
    return knex.raw(
  ...
  average_day_break: ({spaceId, daysAgo}, args, context) => {
    return knex.raw(
      'SELECT  strftime(\'%d/%m/%Y \', datetime(T1.time, \'unixepoch\', \'localtime\')) \n' +
      '    as date,\n' +
      '    Cast ((\n' +
